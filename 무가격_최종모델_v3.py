"""
무 도매 평균가격 순별 예측 모델 (v3 최종 개선판)
==============================================
개선사항:
1. 기상 데이터 병합 전 지역별 독립적 파생변수(Lag) 생성 (기상 충격 방지)
2. 가격 데이터(전년, 평년) 선형 보간 제거 (가격 변동성 보존)
3. 피쳐 선별 시 TimeSeriesSplit 교차검증 기반으로 변경 (Data Leakage 방지)
"""
...
# ================================================================
# 3. 데이터 품질 처리 (보완점 2 적용)
# ================================================================
print("\n--- 데이터 품질 처리 ---")

# ── (1) 가격: 전년·평년 0값 → 선형 보간 제거 (NaN 변환) ──────────
for col in ['전년', '평년']:
    n_zero = (price[col] == 0).sum()
    if n_zero > 0:
        price[col] = price[col].replace(0, np.nan)
        print(f"  [가격] {col}: 0값 {n_zero}개 → NaN 변환 (선형보간 제거)")

# ── (2) 반입량: 전년·평년 0값 → NaN 변환 ──────────────────────
for col in ['전년', '평년']:
    if col in supply.columns:
        n_zero = (supply[col] == 0).sum()
        if n_zero > 0:
            supply[col] = supply[col].replace(0, np.nan)
            print(f"  [반입량] {col}: 0값 {n_zero}개 → NaN 변환 (선형보간 제거)")

# ── (3) 성산 평균_일사량 결측 처리 ─────────────────────────────
seongsan_null = seongsan['평균_일사량'].isna().sum()
if seongsan_null > 240:
    go_solar = gochang.set_index('idx')['평균_일사량']
    seongsan['평균_일사량'] = seongsan['idx'].map(go_solar)
    print(f"  [성산] 평균_일사량: {seongsan_null}행 결측 → 고창군 값으로 대체")
else:
    seongsan['평균_일사량'] = seongsan['평균_일사량'].interpolate(method='linear', limit_direction='both')
    print(f"  [성산] 평균_일사량: 선형 보간")

# ── (4) 기상 데이터 잔여 NaN 선형 보간 (기상은 연속성이 있으므로 유지) ──
for name_str, wdf in [('성산', seongsan), ('고창군', gochang), ('대관령', daegwallyeong)]:
    for col in WEATHER_COLS:
        if col in wdf.columns:
            n_null = wdf[col].isna().sum()
            if n_null > 0:
                wdf[col] = wdf[col].interpolate(method='linear', limit_direction='both')
                print(f"  [{name_str}] {col}: {n_null}개 NaN → 선형 보간")

print("--- 데이터 품질 처리 완료 ---\n")


# ================================================================
# 4. 지역별 기상 파생 변수 선행 생성 (보완점 1 적용)
# ================================================================
def add_weather_features(wdf):
    wdf = wdf.sort_values('idx').copy()
    # 날씨 Lag
    for c in WEATHER_COLS:
        for lag in [1, 3, 6, 9]:
            if c in wdf.columns:
                wdf[f'{c}_l{lag}'] = wdf[c].shift(lag)
    # 날씨 파생 변수
    if '평균_최고기온' in wdf.columns and '평균_최저기온' in wdf.columns:
        wdf['temp_range_l3']  = wdf['평균_최고기온'].shift(3) - wdf['평균_최저기온'].shift(3)
        wdf['heat_stress']    = (wdf['평균_최고기온'].shift(3) > 30).astype(int)
        wdf['cold_stress']    = (wdf['평균_최저기온'].shift(3) < -5).astype(int)
    if '평균_최저초상온도' in wdf.columns:
        wdf['frost_stress']   = (wdf['평균_최저초상온도'].shift(1) < -3).astype(int)
    if '총_강수량' in wdf.columns:
        wdf['heavy_rain_l3']  = (wdf['총_강수량'].shift(3) > 100).astype(int)
        wdf['heavy_rain_l6']  = (wdf['총_강수량'].shift(6) > 100).astype(int)
    return wdf

seongsan = add_weather_features(seongsan)
gochang = add_weather_features(gochang)
daegwallyeong = add_weather_features(daegwallyeong)

# 확장된 기상 컬럼 리스트 추출
EXTENDED_WEATHER_COLS = [c for c in seongsan.columns if c not in ['DATE', 'Year', 'Month', 'Period', 'PeriodStr', 'idx']]


# ================================================================
# 5. 기상 데이터 병합 (계절별 주산지 기준)
# ================================================================
def get_weather_source(month, se_row, go_row, da_row):
    if month in [12, 1, 2]: return se_row
    elif month in [7, 8]: return da_row
    else: return go_row
...
all_idx = sorted(set(se_dict.index) | set(go_dict.index) | set(da_dict.index))
weather_rows = []
...
for idx in all_idx:
    ...
    if src is None: src = next((r for r in [se_row, go_row, da_row] if r is not None), None)

    row = {'idx': idx}
    for c in EXTENDED_WEATHER_COLS:
        row[c] = float(src[c]) if (src is not None and c in src.index and pd.notna(src[c])) else np.nan
    weather_rows.append(row)

weather = pd.DataFrame(weather_rows)
print(f"[기상 병합] {weather.shape[0]}행 생성 (Lag/파생변수 포함 완료)")


# ================================================================
# 6. 전체 데이터 병합 & 추가 피쳐 엔지니어링
# ================================================================
TARGET = '평균가격'

df = price[['idx', 'Year', 'Month', 'Period', 'PeriodStr', '평균가격', '전년', '평년', 'DATE']].copy()
df = df.merge(weather[['idx'] + EXTENDED_WEATHER_COLS], on='idx', how='left')
df = df.merge(supply[['idx', '총반입량']], on='idx', how='left')
df = df.merge(search[['idx', '평균_검색량']], on='idx', how='left')
df = df.sort_values('idx').reset_index(drop=True)

print(f"[전체 병합] {df.shape[0]}행 × {df.shape[1]}열")
...
df['kimchi_season'] = ((df['Month'] >= 10) & (df['Month'] <= 12)).astype(int)
df['winter_mu']     = ((df['Month'] == 12) | (df['Month'] <= 2)).astype(int)
df['spring_mu']     = ((df['Month'] >= 3)  & (df['Month'] <= 6)).astype(int)
df['summer_mu']     = ((df['Month'] >= 7)  & (df['Month'] <= 8)).astype(int)
df['autumn_mu']     = ((df['Month'] >= 9)  & (df['Month'] <= 11)).astype(int)


# ================================================================
# 7. 데이터 정제 + Train/Test 분리
# ================================================================
EXCLUDE = ({'idx', 'Year', 'Month', 'Period', 'PeriodStr', 'DATE', TARGET, '전년', '평년', '총반입량', '평균_검색량'} | set(WEATHER_COLS))
...
# ffill을 통해 앞선 정상 데이터로 결측치 채우기 (가격 변동성 유지)
df_clean[ALL_FEATURES] = df_clean[ALL_FEATURES].ffill().fillna(0).replace([np.inf, -np.inf], 0)
...
# ================================================================
# 8. 피쳐 선별 (교차검증 기반 - 보완점 3 적용)
# ================================================================
print("\n--- 피쳐 선별 (CV 기반 Data Leakage 방지) ---")
...
# 1단계: 전체 피쳐 학습 -> 중요도 산출
selector = xgb.XGBRegressor(n_estimators=500, max_depth=4, learning_rate=0.05, subsample=0.8, colsample_bytree=0.8, random_state=42, verbosity=0)
selector.fit(X_all_tr, y_tr)

importance_df = pd.DataFrame({'feature': ALL_FEATURES, 'importance': selector.feature_importances_})
importance_df = importance_df.sort_values('importance', ascending=False).reset_index(drop=True)

# 2단계: Train set 내에서 TimeSeriesSplit을 사용해 최적 피쳐 수 탐색
tscv_fs = TimeSeriesSplit(n_splits=3)
best_cv_r2, best_n = -999, len(ALL_FEATURES)
search_max = min(56, len(ALL_FEATURES) + 1)

for n_feat in range(20, search_max):
    top_feats = importance_df.head(n_feat)['feature'].tolist()
    X_tmp = train[top_feats].values
    
    fold_r2_list = []
    for tr_idx, val_idx in tscv_fs.split(X_tmp):
        m = xgb.XGBRegressor(n_estimators=300, max_depth=3, learning_rate=0.05, subsample=0.8, colsample_bytree=0.8, random_state=42, verbosity=0)
        m.fit(X_tmp[tr_idx], y_tr[tr_idx])
        p = m.predict(X_tmp[val_idx])
        fold_r2_list.append(r2_score(y_tr[val_idx], p))
        
    avg_cv_r2 = np.mean(fold_r2_list)
    if avg_cv_r2 > best_cv_r2:
        best_cv_r2 = avg_cv_r2
        best_n  = n_feat

SELECTED = importance_df.head(best_n)['feature'].tolist()
print(f"  최적 피쳐 수: {best_n}개 (탐색 CV R²={best_cv_r2:.4f})")
...
# ================================================================
# 10. 교차검증 (TimeSeriesSplit 5-fold)
# ================================================================
...
metrics = {
    ...
    "data_quality_fixes": {
        "가격_전년_평년_0값":    "NaN 변환 후 ffill 보간 (변동성 유지)",
        "반입량_전년_평년_0값":    "NaN 변환",
        "성산_평균_일사량_결측":   "고창군 일사량으로 대체",
    },
    "weather_mapping": {
        "겨울무(12~2월)": "성산",
        "봄무(3~6월)":    "고창군",
