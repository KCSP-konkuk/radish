...
# ── (1) 가격: 전년·평년 0값 → NaN → 선형 보간 ──────────────────
# 2018년 전년(2017) 데이터 없음 → 0으로 채워짐 (36행)
# 2018~2022년 평년(5개년 평균) 산출 불가 → 0으로 채워짐 (180행)
for col in ['전년', '평년']:
    n_zero = (price[col] == 0).sum()
    if n_zero > 0:
        price[col] = price[col].replace(0, np.nan)
        price[col] = price[col].interpolate(method='linear', limit_direction='both')
        print(f"  [가격] {col}: 0값 {n_zero}개 → NaN 변환 후 선형 보간")

# ── (2) 반입량: 전년·평년 0값 → NaN → 선형 보간 ─────────────────
for col in ['전년', '평년']:
    if col in supply.columns:
        n_zero = (supply[col] == 0).sum()
        if n_zero > 0:
            supply[col] = supply[col].replace(0, np.nan)
            supply[col] = supply[col].interpolate(method='linear', limit_direction='both')
            print(f"  [반입량] {col}: 0값 {n_zero}개 → NaN 변환 후 선형 보간")

# ── (3) 성산 평균_일사량: 287/288행 결측 → 고창군 값으로 대체 ───
# 성산(제주) 기상청 일사량 관측 미제공
...
if seongsan_null > 240:
    ...
    print(f"  [성산] 평균_일사량: {seongsan_null}행 결측 → 선형 보간")

# ── (4) 기상 강수량 0값 → 실제 강수 없음, 정상 처리 유지 ────────
print("  [기상] 총_강수량 0값: 강수 없음으로 정상 처리")

# ── (5) 기상 데이터 잔여 NaN → 선형 보간 ────────────────────────
...
metrics = {
    ...
    "data_quality_fixes": {
        "가격_전년_0값(36개)":     "NaN 변환 후 선형 보간",
        "가격_평년_0값(180개)":    "NaN 변환 후 선형 보간",
        "반입량_전년_평년_0값":    "NaN 변환 후 선형 보간",
        "성산_평균_일사량_결측":   "고창군 일사량으로 대체",
        "기상_강수량_0값":         "실제 강수 없음으로 정상 처리",
    },
    "weather_mapping": {
        "겨울무(12~2월)": "성산",
        "봄무(3~6월)":    "고창군",
